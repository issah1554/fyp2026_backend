...
def seed_market_integration_permissions(apps, schema_editor):
    Permission = apps.get_model("users", "Permission")
    Role = apps.get_model("users", "Role")
    RolePermission = apps.get_model("users", "RolePermission")

    permissions_by_code = {}
    target_codes = {"market_integrations.read", "market_integrations.write"}
    ...
    for role_code, permission_codes in DEFAULT_ROLE_PERMISSIONS.items():
        ...
        for permission_code in permission_codes:
            if permission_code in target_codes and permission_code in permissions_by_code:
                if not RolePermission.objects.filter(role=role, permission=permissions_by_code[permission_code]).exists():
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(seed_market_integration_permissions, migrations.RunPython.noop),
    ]
